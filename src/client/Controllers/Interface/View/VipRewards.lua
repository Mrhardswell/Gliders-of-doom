...
function VipRewards.new(ScreenGui, Interface)
    local self = {}
    ...
    local function CheckVip()
        local isVip = MarketPlaceService:UserOwnsGamePassAsync(Player.UserId, VIPID)
        if isVip then
            self.Button.Label.Text = "CLAIM"
            self.Button.ImageColor3 = Color3.fromRGB(255, 255, 255)
        else
            self.Button.Label.Text = "BUY VIP"
            self.Button.ImageColor3 = Color3.fromRGB(255, 0, 0)
        end
    end

    Net:Connect("PromptTriggered", CheckVip)
    ...
    self.Button.MouseButton1Click:Connect(function()
        ...
        local isVip = MarketPlaceService:UserOwnsGamePassAsync(Player.UserId, VIPID)
        if isVip then
            self.Merchants:ClaimReward("VIP")
        else
    ...
    MarketPlaceService.PromptGamePassPurchaseFinished:Connect(function(Player, GamePassId, PurchaseSuccess)
        if GamePassId == VIPID and PurchaseSuccess then
            print("VIP Purchased")
            self.Button.Label.Text = "CLAIM"
            self.Button.ImageColor3 = Color3.fromRGB(255, 255, 255)
        end
